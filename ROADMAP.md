...
**Updated:** 2026-09-24 06:45 (window closeout: offsite-borg guard stack verified complete — drill fixture, §13/§16 smoke fixtures + positive-render eval check landed; the Theme-1 off-site row's pre-go-live list refreshed: restore drill + deploy smokes DONE, VM test + recovery-copy policy remain) (previous: 2026-09-23 02:01 (window closeout: off-site backup IMPLEMENTED dormant — trigger language refreshed; restic idea trigger now the first nightly runs post system-796) (previous: 2026-09-22 05:07 (docs-health window closeout: restic single-source-of-truth idea added to Theme 1) (previous: 2026-09-19 13:40 (docs-health pass: AppArmor + disabled-service duplicate bullets merged — AppArmor rejection reason corrected to the paepckehh profile-fit verdict; stale VM-test counts 11→43) (previous: 2026-09-14 18:30 docs-health AUDIT pass: added the IO-telemetry + agent-session IO ideas from the freeze-4 window; Zone 7 episodic-IO guard idea; offsite Borg leg decided — moved to TODO implementation)
...
- **Off-site backup** — **pool safety net is LIVE (2026-08-17):** btrbk root+data sends + every application backup (forgejo, pocket-id, immich, twenty, manifest, paperless) land on the 2×16 TB BTRFS mirror — the single-NVMe risk is closed. The 3rd, OFF-SITE copy is **DECIDED (2026-09-11): Hetzner StorageBox BX11 + BorgBackup** (repokey-blake2, port 23; rejected: Google Photos/Drive — `google-sync` DORMANT + mirrors live data not snapshots — and sdf vault rotation — drive FROZEN, unbounded RPO). Rationale + blueprint: `docs/research/hetzner-storagebox-borgbackup.md`. IMPLEMENTED 2026-09-22 — deployed DORMANT (`services.offsite-borg.enable = false`, system-796); go-live is owner-gated on the StorageBox hostname/username + host-key pin; agent-side pre-go-live work is DONE as of 2026-09-24 (restore drill + drill fixture, §13/§16 deploy-smoke fixtures, positive-render eval guard, runbook accuracy pass) — remaining before flip: `backup.nix` VM test, recovery-copy policy decision, EIO `/data` inode repair-vs-exclude (runbook: `docs/services/offsite-borg.md`)
- **Restic as the single source of truth for app-dump retention (idea, 2026-09-22)** — IF the restic-app-dumps repo proves itself after ≥3 nightly runs (dedup ratio measured, restore smoke green), evaluate replacing the per-service 14d file-retention rotations with restic-managed retention; until measured, the two layers coexist deliberately (per-service freshness rows stay the tripwire). Trigger: first nightly runs landed (system-796, 2026-09-22); proof chain = TODO_LIST restic item
...
- **`/data` rename + model-root consolidation (user idea 2026-09-05)** — `/data` holds ONLY redownloadable bulk (ai 291G + models 210G + llamacpp 92G + Steam 106G + docker root ~20G, mostly overlay2); the sole stateful data is the docker data-root's tiny volumes (twenty/manifest postgres), ALREADY covered pool-side by nightly pg_dumps, PLUS Steam `compatdata` (2.5 G, 3 Proton prefixes — CS2 is cloud-only; NFS Hot Pursuit Remastered has confirmed LOCAL save paths under `pfx/.../Documents` + `Saved Games`; NFS HPRE/Upload Labs cloud status unverified) — so the broken `/data` btrbk send is nearly irrelevant data-wise (the P0 item is about the corruption itself, not backup value). Rename the mountpoint to a truth-in-advertising name (e.g. `/redownloadable`) and consolidate the THREE scattered model roots into one dedicated folder (`/data/ai/models` is canonical per ai-models.nix; absorb `/data/models` ad-hoc diffusion/video models + `/data/llamacpp-models` GGUFs), carving out a backed-up `saves/` set (compatdata + docker volumes) from the redownloadable bulk first — that is what makes the name honest. Config touchpoints: hardware-configuration.nix, snapshots.nix (btrbk-data volume + tmpfiles + RequiresMountsFor), default-services.nix (docker data-root), ai-models.nix, fastflowlm.nix + llama-rag.nix modelDir defaults, home.nix (Jan symlink), gatus alert texts; runtime steps: docker data-root move (docker down), Steam library re-add, model-path migrations (same-fs renames = instant). Coordinate with the Samsung migration (docker data-root end-state) and the `/data` EIO repair (P0)
- **io.latency pilot for interactive cgroups (research note, LWN 824855 2026-09-06)** — io.cost/io.latency are Btrfs-only cgroup writeback controllers (mutually exclusive with BFQ, which we run); io.latency sets measured-latency hard targets per cgroup, io.cost a work-conserving cost model. Verdict for this box: QLC SLC-cache bimodality (fast cache hits vs ~253 ms direct writes) defeats io.cost model calibration — **io.latency is the only viable pilot** (never io.cost on this hardware), via `IODeviceLatencyTargetSec=` on interactive cgroups (sshd/user slices). TRIGGER: freeze-class recurrence post-Samsung migration. Reference: https://lwn.net/Articles/824855/ (also the source for the snapshot-drop pattern above and the csum-error trust policy)
...
- **Continuous per-unit IO telemetry (freeze-4 idea, 2026-09-14)** — `systemd-run --property=IOAccounting=1` + io.stat sampled by a textfile collector so "who is reading the QLC" is a metric query, not a forensics bundle; pairs with `io-psi-forensics.sh` (event-driven) as the continuous half. Trigger: next Zone 6 storm cycle with unattributed load
- **Guard Zone 7 — episodic IO bucket (idea only, 2026-09-14)** — a leaky-bucket zone for io-pressure avg10 spikes (the Zone 5 analogue for IO), IF live telemetry shows the episodic shape exists; do NOT build preemptively — calibrate from real `io-psi-forensics` data first
- **One-timeline diagnosis view (freeze-4 idea)** — SigNoz dashboard overlaying io/memory PSI + guard trip counters + deploy markers on one time axis; the 2026-09-14 freeze forensics would have been a 2-minute read
- **Agent-session IO admission (freeze-4 idea)** — 4-5 concurrent crush sessions each churn their QLC-root DBs at desktop IO priority; consider demoting batch/parallel agent sessions to a background ioTier (the wrapper is BE/3 "desktop" — fine interactively, wrong for a 5-session storm). Structural fix is the crush-DB-off-QLC migration (TODO P1); this is the priority half
- **disko at the already-deferred reinstall** — GEOMETRY SPEC LANDED 2026-09-14/23 (`disko/samsung-tlc.nix` + eval guard + VM rehearsal, runbook `docs/services/disko.md`); still deferred: the nixos-anywhere/autoinstaller path and the reinstall trigger itself (owner decision)
...
- **Derive `dns-local.nix` subdomains from the integration registry** — `services.integration.*.subdomain` declarations already exist per service (health-dashboard is the worked example); generating the local DNS zone from the registry deletes the two-file split-brain class (subdomain declared but DNS record forgotten) entirely instead of asserting it at eval time. Fleet-level design change; touches dnsblockd's zone consumption. **Source:** docs/status/2026-09-19_19-17_health-home-lan-federation-hub-build.md §f.30
