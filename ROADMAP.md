...
- **Restic as the single source of truth for app-dump retention (idea, 2026-09-22)** — IF the restic-app-dumps repo proves itself after ≥3 nightly runs (dedup ratio measured, restore smoke green), evaluate replacing the per-service 14d file-retention rotations with restic-managed retention; until measured, the two layers coexist deliberately (per-service freshness rows stay the tripwire). Trigger: restic first-runs post-deploy
