...
# surface that proves each guard fires on its incident shape:
#
...
#   4. entries declared with enable=false THROWS.
#   5. Duplicate entry paths THROW.
...
let
  ...
  throws =
    extraModules:
    let
      ok = builtins.tryEval ((evalConfig extraModules).system.build.toplevel.drvPath);
    in
    !ok.success;
