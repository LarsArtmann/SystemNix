...
- [x] **SigNoz: 19 alert rules provisioned** — Fixed 2026-07-30. The v5 API format was already deployed (2026-07-29), but 4 rules used `target=0` with `above_or_equal` (always true), causing 3 rules to permanently fire. Fixed all 4 rules to `target=1`: `service-down`, `nvme-critical-warning`, `nvme-media-errors`, `dnsblockd-crashes` (also changed query from `rate(...) > 0` to `increase(...)`). Provision script now logs HTTP response bodies on failure (v4). All 19 rules verified: 0 errors, all `state: inactive`.
