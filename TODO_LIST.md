...
**Updated:** 2026-08-14 | **Last sessions:** vendorHash drift checks across 11 LarsArtmann Go repos (5 stale hashes + 1 missing publicDeps caught on first run — all fixed); systemd hardening primitives added to `harden()` + I/O-throttled dev wrappers (`go-build-memlimit`, `cargo-build-memlimit`); GOMEMLIMIT validation script + SigNoz collector 384MiB→768MiB fix; PMA `GenerateMessage` verified already-fixed; go-auto-upgrade re-enabled (vendoring fixed upstream); docs harvest of all 2026-08-14 status reports; SigNoz dashboard v2 migration research (not started); browser-history registration lock (cqrs-htmx `MaxUsers` gate — OAuth2 bypass + TOCTOU CLOSED upstream, release still open) + oomd threshold raise 50%/20s→60%/30s; smart-audio daemon + qmd retirement + Twenty hardening; monitoring gap closures, mkOidcGate/mkDnsGate refactors
...
- [x] **VendorHash CI check across LarsArtmann repos** — DONE: Replicated dnsblockd's `vendor-hash` FOD-realization check (interpolates `.goModules` into a runCommand — stale hashes fail fast without compiling) across 11 repos: browser-history (server+agent), crush-daily (via mkGoFlake extraChecks), file-and-image-renamer (main+filechange), DiscordSync (discordsync+cqrs-lint), art-dupl, branching-flow, go-cqrs-lite (cqrs-lint+benchstat), go-humanize-linter, go-structure-linter, project-meta, projects-management-automation. **The checks immediately caught 5 genuine stale hashes** (browser-history server+agent vendorHash, filechange vendorHash, benchstat source hash + vendorHash) plus a missing publicDeps entry (go-codec in browser-history) — all fixed with got: hashes. All 15 checks verified building green
- [x] **GOMEMLIMIT runtime validation** — DONE (static part): Fixed SigNoz OTel collector anomaly (384MiB was 37.5% of MemoryMax; normalized to 768MiB = 75%, matching query service pattern). Created `scripts/validate-gomemlimit.sh` — post-deploy one-command validation comparing MemoryCurrent vs MemoryMax (cgroup level) and go_memstats_heap_inuse_bytes vs GOMEMLIMIT (Go heap level, where /metrics exposes go runtime stats; browser-history:8087 confirmed live). **Remaining (manual):** run `sudo bash scripts/validate-gomemlimit.sh` after next deploy
...
- [ ] **cqrs-htmx/browser-history: registration-lock polish (security gap CLOSED 2026-08-14)** — The OAuth2 (Pocket ID) first-login auto-provisioning bypass and the advisory count check (TOCTOU) are fixed in cqrs-htmx: `OAuth2Service.matchOrCreateUser` enforces the same `MaxUsers` cap before dispatching `RegisterUserCmd` (existing users always log in; error `usermgmt.oauth2.registration_closed`, HTTP 403 — handler-tested), and `Service.Register` + the OAuth2 path share a registration mutex across the check-then-dispatch window (TOCTOU-safe for in-process sync projections). Remaining NON-security follow-ups: the frontend register form still renders and surfaces a raw 403 (needs friendly "registration closed" state); no monitoring alert if browser-history user count exceeds 1 (would detect ANY future bypass — needs a user-count metric from browser-history first). **Source:** `docs/status/2026-08-14_10-04_registration-lock-oomd-threshold-session.md`
- [x] **cqrs-htmx/browser-history repo hygiene** — Full browser-history test suite run 2026-08-14 (`go test ./...` — pass; only `cmd/*` lack test files); `MAX_USERS` documented in browser-history README env table + `docs/configuration.md`; cqrs-htmx `context_actor_test.go` verified gofmt-clean (already formatted)
