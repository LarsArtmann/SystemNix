...
- [x] **Fix the pre-existing `checks.disko-layout` eval failure (`disko-samsung-tlc-vm.nix.drv is not valid`) — the pre-commit flake-check gate is dead for ALL agents until this lands** → [docs/todo/pipeline.md](docs/todo/pipeline.md) — DONE 2026-09-25 (task queue, attempts 1+2): the gate was environmental, NOT structurally dead — root cause = the `hot-user-caches-nix-bootstrap` ↔ `systemd-tmpfiles-setup` boot-ordering cycle (stability row below, TODO 109) deleting boot tmpfiles → `/run/binfmt` missing → every sandbox build fails; gate verified GREEN 3× on 2026-09-25 (05:33, 06:34, 06:55 pre-commit-hook-verified at `4435ddb8`) and the delta since is docs-only. Durable fixes IN-TREE: `af87ce31` (bootstrap → `.mount` anchoring + `tests/test-hot-user-caches.nix` tmpfiles tripwire) + `d827f478` (`binfmt-sandbox-dir` belt) + the flake.nix formatter override whose eval no longer coerces the treefmt package output (the `treefmt.drv is not valid` shape). RESIDUAL NOT closed by this [x] — owner-gated: the durable fixes are UNDEPLOYED (host still boots gen 797 with the old automount-anchored wiring, re-probed 07:12), so a reboot BEFORE the owner deploy re-manifests the cycle; post-reboot gate proof is tracked `[blocked:deploy]` on the pipeline row + stability row 112 (quiet-window VM test → deploy → reboot-verify; re-check then: `ls /run/binfmt && nix flake check --no-build`). Attempt-2 full re-check was doctrine-skipped (IO PSI some avg60 91.75% storm) — CORRECTION 07:26: the pre-commit hook ran the FULL check GREEN twice on this close's final tree (~07:14, ~07:17) while landing the commits, so the day's count is 5 greens, not 3; the skip held for manual probes only. Full chain: docs/status/2026-09-25_06-45/06-58 + 07-12/07-26 task reports
...
- [x] **Build the /data damage-set inventory** → [docs/todo/storage.md](docs/todo/storage.md) — DONE 2026-09-25 (task queue): `docs/services/data-damage-set.md` (victims 1 gone + 10 live ~49G, scrub/gate evidence, snapshot-pinning timeline CONSUMING the AGENTS.md doctrine table — cited, never duplicated; now a live surface of the jan.md recipe's doc sweep). The inventory CONSUMES the AGENTS.md snapshot-pinning doctrine table for its timeline (cites it, never duplicates it) (Source: 2026-09-25 sweep report §b4)
- [x] **Cross-link the flm AGENTS bullet to the snapshot-pinning doctrine table (one-line symmetry with jan.md)** → [docs/todo/storage.md](docs/todo/storage.md) — DONE 2026-09-25 (task queue): the flm Package bullet's re-pull sentence now carries the caveat (in-place overwrite pins old+new weights through the ~4-5w /data window) with a pointer to the BTRFS "Snapshot-pinning doctrine" table (Source: docs/status/2026-09-25_00-20_task-000001a0d56f35ccee2a991988d7c3a9ebd1.md §f2)
- [x] **Refresh the doctrine table's `/data/docker` sizing via `docker system df` (docker group, no sudo)** → [docs/todo/storage.md](docs/todo/storage.md) (Source: same report §f4)
...
- [ ] **One-sweep AGENTS.md pointer audit: every service bullet mentioning in-place overwrite/delete of a /data-resident artifact gets a snapshot-pinning doctrine pointer (subsumes the second flm "Known crash" bullet's missing link)** → [docs/todo/storage.md](docs/todo/storage.md) (Source: docs/status/2026-09-25_10-41_task-000001a0d7affe0ef79cf96e03be8e0247ca.md §e4/§f1-2)
- [ ] **Inventory hardening: inline re-verification one-liners (`stat -c %i` per victim row) + a freshness tripwire in docs/services/data-damage-set.md** → [docs/todo/storage.md](docs/todo/storage.md) (Source: docs/status/2026-09-25_10-18_task-000001a0d79daeca24808d2e967271007d42.md §e1-2)
- [ ] **Reconcile the "~12 journal pairs vs 11 enumerated" /data journal residual explicitly, or downgrade the P0 row's "~12" wording** → [docs/todo/storage.md](docs/todo/storage.md) (Source: docs/status/2026-09-25_10-18_task-000001a0d79daeca24808d2e967271007d42.md §b3)
- [ ] **Decide `services.offsite-borg.enable` plain `false` vs `lib.mkDefault false` (mkDefault kills the plain-100 probe-conflict class; plain keeps the go-live flip a conscious edit) — BLOCKED: semantics/ownership call that decides whether future probes need `mkOverride 50` forever** → [docs/todo/storage.md](docs/todo/storage.md) (Source: docs/status/2026-09-25_11-30_five-task-window-closeout §g1)
...
- [ ] **Extend CI's shellcheck job to `scripts/lib/*.sh` (find -print0 | xargs -0, keep `--severity=error`) + add `.githooks/*` to the hook's own shellcheck leg — closes the error-bar blind spot on the deploy-gate lib** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-24_07-31_task-000001a0d1a502e30b7e95d3ecda2ac1b29b.md §b1/§fN1+N5)
- [ ] **mktemp (+trap rm) the pre-commit hook's per-leg logs (`/tmp/shellcheck.log`, `/tmp/ruff.log`) — fixed paths race under concurrent commits in this multi-agent repo (exit decisions unaffected; the printed log lies)** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-24_07-31_task-000001a0d1a502e30b7e95d3ecda2ac1b29b.md §d3/§fN4)
- [ ] **Add the extendModules override recipe to docs/CONTRIBUTING.md's Eval-Time Guards section (mkOverride-50 template, throwaway-eval command, priority ladder: ≤50 beats plain-100, <50 beats mkForce)** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-24_06-03_task-000001a0d1898ba3e48695371bc62baac77d.md §b1/§f2)
- [ ] **CI early warning: scheduled clean-HEAD `nix flake check --no-build` job — the /run/binfmt gate death sat ~2 days and would have been caught in hours** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-25_09-52_task-000001a0d6d4440f02adfdb7dddfd259e173.md §f49)
- [ ] **Shared queue-agent pre-run preamble script: PSI-threshold probe (/proc/pressure/io) + docs-only-delta check — makes the IO-storm skip rule and the "gate safety unchanged" argument mechanical instead of judgment-based** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-25_11-30_five-task-window-closeout §e6/§f8)
- [ ] **Promote `boot.binfmt.preferStaticEmulators = true` to a real config change after the boot-ordering deploy proves out (removes `/run/binfmt` from extra-sandbox-paths, killing the whole sandbox-build class)** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-25_09-52_task-000001a0d6d4440f02adfdb7dddfd259e173.md §f33)
- [ ] **Codify the fleet-incident rule in docs/CONTRIBUTING.md: a gate failing for ALL agents is an environment incident (the /run/binfmt saga as case study), triaged before touching any individual task** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-25_09-52_task-000001a0d6d4440f02adfdb7dddfd259e173.md §f50)
- [ ] **Hook shellcheck source: pin from this flake (hermetic, no per-commit nixpkgs eval/network) vs status-quo `nix shell nixpkgs#shellcheck` (always-current) — BLOCKED: latency-vs-freshness tradeoff is an owner call** → [docs/todo/pipeline.md](docs/todo/pipeline.md) (Source: docs/status/2026-09-24_07-31_task-000001a0d1a502e30b7e95d3ecda2ac1b29b.md §g3)
