...
- [x] ~~**Reconcile the root `@` retention anomaly (12-day-old weekly vs `snapshot_preserve 3d 1w`) and restate the doctrine's root window**~~ **DONE 2026-09-25:** btrbk 0.32.7 `schedule()` source-verified — buckets are CALENDAR-anchored (weeks start Sunday); `1w` keeps TWO weeklies (current + previous calendar week's first snapshot), so a Sunday-weekly lives exactly 14 days; the 12-day-old weekly was in-spec. Doctrine restated in AGENTS.md: root `@` pin window = 2w sharp. → [docs/todo/storage.md](docs/todo/storage.md) (Source: same report §g1/f7)
