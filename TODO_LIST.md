...
- [ ] **Evaluate oomd pressure threshold** — Current `DefaultMemoryPressureLimit = 50%` / `DefaultMemoryPressureDurationSec = 20s` is too aggressive. **Verified Aug 14:** system-slice memory pressure sustained at 65% (some avg10) due to PMA page-cache reclaim (6.6 GiB, 2.9M MemoryHigh events). oomd killed dnsblockd 730 times today (1,591 since boot) — DNS breaks for all LAN clients each time. IO PSI at 65% (6.5x WDT crash threshold). Consider 60%/30s or per-slice config. **Source:** `docs/status/2026-08-14_08-46_monitoring-gap-closures-self-review.md`
