...
- [ ] **BTRFS root chunk-unalloc CRITICAL (~6.4 GiB) — run metadata balance when IO settles + harden the reserve** — both scheduled balance jobs self-skipped correctly during the DAS/freeze era; the 2026-08-31 gawk fix (exit-127 class, dead for a week) restored them. Runbook (embedded in gc-guard + balance skip messages): (1) `rm /btrfs-emergency-reserve` FIRST, (2) wait for QUIET (zram <80%, IO PSI <20%), (3) `sudo ionice -c 3 btrfs balance start -musage=50 /` (+ `-dusage=5 -dlimit=2` if needed), (4) re-provision the reserve (`sudo systemctl start btrfs-emergency-reserve`). Remaining from old item: the reserve file's extents are snapshot-pinned — either a periodic rewrite timer or a documented pinning caveat in AGENTS.md (plan T14). After balance: re-check unalloc >10G before declaring safe. **Source:** `docs/status/2026-08-31_20-02_disk-review-samsung-assignment-nix-fs-research.md` §f.3/7
...
- [ ] **Mail Relay go-live (user steps; module built + enabled with PLACEHOLDER credential 2026-09-02)** — central Postfix null client on 127.0.0.1:25 → Resend submission (`services.mail-relay`); paperless + forgejo + system mail wired and relay-gated. Until go-live every send defers in the mailq (inert, nothing crashes). Steps (runbook `docs/services/mail-relay.md`): (1) new Resend API key (same key fixes Pocket ID, see PERSISTENT NAG), (2) verify a sending domain, (3) `sudo sops platforms/nixos/secrets/mail-relay.yaml` (replace `mail_relay_password` PLACEHOLDER) + restart postfix, (4) set `services.mail-relay.fromAddress` to the verified domain + deploy, (5) sendmail E2E test + paperless share-link test, (6) optionally set Immich SMTP in its admin UI (127.0.0.1:25, no auth), (7) decide the Paperless INBOUND mailbox (Mail account via UI, Gmail app password) + a Mail rule. **Source:** mail-relay session 2026-09-02
...
- [ ] **profileProbe enable decision (user)** — `services.cv-server.profileProbe` OFF by default; enabling arms the weekly session probe + exit-3 onFailure alerting. **Source:** 08-30 11-15 §f.15
...
- [ ] **test-cv latent `fileSystems` bug — pool was never mounted in the VM** — cv test's `fileSystems."/mnt/pool"` silently vanished from the guest fstab (qemu-vm.nix `mkVMOverride` replaces the whole option; VM tests need `virtualisation.fileSystems`) — cv-backup assertions ran against a root-fs directory, weaker than believed. Convert to `virtualisation.fileSystems`, re-verify cv-backup under a REAL btrfs mount (its mount-gated dir oneshot + ReadWritePaths path will now fully activate). Reference implementation: `tests/test-pool-recovery.nix`. **Source:** `2026-08-31` pool-recovery VM-test debug session
...
- [ ] **Wire `scripts/audit-go-deps.sh` into pre-deploy/CI** — script shipped + first run clean (1569 OK / 46 WARN-DIVERGED); wiring pending the WARN-DIVERGED policy decision (owner Q1-Q3 from the 05-30 report, never answered)
- [ ] **nullglob audit of ALL `runCommand` checks** — unquoted command-variable expansion + `shopt nullglob` = silent phantom-green (caught once in signoz-query-lint v1); `grep -rn '\$[a-z]* "\?'`-style sweep. **Source:** 08-27 18-58 §f.10
- [ ] **Persist the nix-native negative-test harness as a repo script** (store fixtures + `builtins.replaceStrings` mutation — currently only described in reports). **Source:** 08-27 18-58 §e.8
