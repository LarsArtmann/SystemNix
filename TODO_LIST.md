...
- [x] **BuildFlow: pre-commit needs missing devShell binaries — DONE 2026-09-02** — three defects fixed upstream (BuildFlow repo): (1) default devShell lacked `buildflow` itself, so the hook resolved a stale system binary (the exact class the `.#reinstall` app documents) — added to the shell; (2) `buildflow precommit install`/`uninstall`/`status`/`setup` hardcoded `.git/hooks`, ignoring `core.hooksPath` — evo-x2 has `core.hooksPath=.githooks`, so the installed hook was DEAD (git executed nothing; silent no-hook state); the installer is now hooksPath-aware (`git config --local core.hooksPath`, falling back to `.git/hooks`; `--local` deliberately — a merged read leaked this machine's GLOBAL `.githooks` into fixtures); (3) regression test added (`honors repo-local core.hooksPath`, real `git init` fixture). Full `internal/cli` + `discovery` suites green, `nix flake check` green. FOLLOW-UP (owner): run `nix run .#reinstall && buildflow precommit install` once to materialize `.githooks/pre-commit` with the fixed binary — the hook is still not materialized on evo-x2. **Source:** TODO sweep session 2026-09-02
