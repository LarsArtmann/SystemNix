...
- [ ] **InboxClean → Paperless archiving go-live (user steps; wiring shipped + enable-gated 2026-09-02)** — Gmail attachments (16 formats) + opt-in `.eml` bodies auto-archive into Paperless after every InboxClean sync (ledger + checksum dedup = idempotent). Everything is wired (sops template, env wiring, Gatus "InboxClean Paperless Archive Auth", post-deploy smoke, `tests/test-inboxclean-paperless.nix`) and gated behind `services.inboxclean.paperless.enable = false`. Go-live: (1) `sudo -u paperless paperless-manage drf_create_token admin`, (2) `sudo sops platforms/nixos/secrets/inboxclean-paperless.yaml` (replace PLACEHOLDER), (3) flip `paperless.enable = true` in configuration.nix + deploy — (2) and (3) MUST land together: URL-without-token is a config Rejection at every inboxclean start (web + sync crash). Verify: `journalctl -u inboxclean-sync | grep -i paperless`, /sync dashboard card, Gatus check green. **Source:** inboxclean-paperless session 2026-09-02
- [ ] **Mail relay hardening finish (agent; collector shipped, check missing)** — the Gatus "Mail Relay Queue" check edit failed silently mid-session 2026-09-02 (`mail-relay-metrics` collector + queue/placeholder/scrape_errors gauges ARE shipped; the consuming check is NOT). Finish: re-add anchored check, `tests/test-mail-relay.nix` (loopback-only bind, 220 banner, submit+defer under placeholder, metrics file), post-deploy-check §12 (banner + placeholder WARN + paperless env), docs. **Source:** mail-relay session 2026-09-02
