...
- [x] **Add `system_oomd_kills_total` metric** — DONE: Added `system_oomd_kills_total`, `system_oomd_kills_recent`, and `system_oomd_kills_alert` textfile metrics in `system-health.nix` (counts systemd-oomd kill events from journal with delta tracking). "OOMD Kills" Gatus alert added in `gatus-config.nix`. **Grep pattern verified on live system:** `"Killed"` returned 0 matches (wrong), fixed to `"Marked.*for killing"` (2,203 matches since boot). Live data shows dnsblockd is the primary oomd victim (1,591 kills)
...
- [x] **Browser-history: registration lock after first user** — DONE: Added `MaxUsers` config to cqrs-htmx `ServiceConfig` (0=unlimited, 1=single-user). Gate in `Service.Register()` checks `readModel.Count() >= maxUsers` and returns `ErrRegistrationClosed` (HTTP 403). Wired in browser-history via `MAX_USERS=1` env (default 1). SystemNix `browser-history.nix` sets `MAX_USERS=1` explicitly. Tests added in `service_register_test.go` (3 cases: max reached, unlimited, multi-user). Upstream repos: cqrs-htmx (identity-model/errors.go, usermgmt/{errors,service_core,service_register}.go) + browser-history (api/{config,server}.go, go.work)
- [x] **Evaluate oomd pressure threshold** — DONE: Raised `DefaultMemoryPressureLimit` from 50% to 60% and `DefaultMemoryPressureDurationSec` from 20s to 30s in `boot.nix`. Per-slice `ManagedOOMMemoryPressureLimit` also raised from 50% to 60% for root/system/user slices. The previous 50%/20s killed nix-daemon mid-build (65% pressure during legitimate 4-8 GB build) and Twenty worker in steady-state. 60%/30s tolerates transient pressure from nix builds, AI model loads, and Docker restarts while still catching genuine memory exhaustion. nix-daemon already exempted via `ManagedOOMPreference=omit`; per-slice `MemoryMax` limits remain as backstop
...
- [ ] **Release + deploy registration lock and go.work fix** — The `MaxUsers` registration gate (cqrs-htmx: identity-model/errors.go, usermgmt/{errors,service_core,service_register,service_register_test}.go; browser-history: api/{config,server}.go) and the browser-history `go.work` identity-model replace fix exist only as uncommitted working-tree changes in the upstream repos. Steps: commit both repos, tag cqrs-htmx (identity-model + usermgmt consumers) and browser-history, bump SystemNix `browser-history` flake input, deploy, then verify `POST /auth/register` returns 403 while logged-out. Note: pre-existing upstream test breakage in `cqrs-htmx/usermgmt/es_materialize_adapter_test.go` (drift vs local go-cqrs-lite: missing `DeleteTypes`/`DeleteInclude`) — fix or exclude before tagging
...
- [x] **Deploy pending changes** — DONE: All accumulated changes deployed via `nh os switch .` (Manifest postgres memory limits, Dozzle memory limits, Pocket ID retries, vendorHash check, test-home-manager.sh fix, monitoring gap closures, mkOidcGate/mkDnsGate refactors, qmd removal, smart-audio daemon, Twenty hardening)
...
- [x] **PMA `GenerateMessage` handler leak** — DONE: Verified already fixed. `GenerateMessage()` (committer.go:279) already has `defer coreutils.CloseQuiet(commitHandler)`, same as the `Commit()` fix. The TODO was stale — the fix was applied in a prior session
