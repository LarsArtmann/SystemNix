...
| Browser History                       | ✅     | `browser-history.nix`                | Go CQRS/ES browser history server on port 8087 (`history.home.lan`). SQLite WAL, WebAuthn + OAuth2 (Pocket ID) dual auth. Direct TLS proxy (NOT `protectedVHost`). Agent extracts history from Firefox/Chromium profiles. OIDC oneshot via `LoadCredential` |
...
| Browser History   | Deployed and healthy (2,927 events). Known gaps: OTel traces not reaching SigNoz (gRPC endpoint may need URL scheme fix), OAuth2 login not manually tested end-to-end. Backup-coordination and agent `after` dependency resolved | Low      |
