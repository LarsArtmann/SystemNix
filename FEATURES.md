...
**Generated:** 2026-05-03 | **Updated:** 2026-08-12 | **Scope:** Full codebase scan
...
| flake-parts modular architecture          | ✅     | 49 modules auto-discovered (54 files total, 5 `_`-prefixed helpers skipped). Run: `nix eval .#nixosModules --apply 'x: builtins.length (builtins.attrNames x)'` |
...
| Twenty CRM                            | ⚠️     | `twenty.nix`                         | Docker Compose (4 containers), PostgreSQL + Redis, sops secrets, daily DB backup, Caddy at crm.home.lan (Layer 2 Pocket ID auth via oauth2-proxy; LAN bypass). Worker has `mem_limit=2g` + `NODE_OPTIONS=--max-old-space-size=1536` (prevents oomd kill loop). **Auth note:** native OIDC/SAML is a Twenty paid entitlement, so no in-app "Login with Pocket ID" exists. **Pending:** Nix memory-limit config still needs deploy to become authoritative.                                                                                                                                                                                          |
...
|| Gatus (health checks)                 | ✅     | `gatus-config.nix`                   | 83 health check endpoints (including nix-daemon liveness), Discord alerting, SQLite storage, port 9110, `status.home.lan`                                                                                                                                                                                                                                                                                    |
...
| Hermes AI gateway                | ✅         | `hermes.nix`       | v0.19 — Discord bot, cron, messaging, edge-tts, exa, firecrawl, fal — system service, sops secrets, 4G memory limit, USR1 reload, multi-provider LLM wiring (GLM, MiniMax, Xiaomi, Synthetic, FAL) |
...
| Browser History   | StartLimit crash-loop fixed (`[Unit]` placement). Known gaps: 4-min startup (no `CheckpointStore`), `expires_at` session reaper schema error, OAuth2 login not manually tested. OTel endpoint scheme fix staged | Medium   |
...
| Gatus health endpoints     | 83    | `grep -c '^\s*name =' modules/nixos/services/gatus-config.nix`               |
