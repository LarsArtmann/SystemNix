...
| Paperless-ngx (document mgmt)         | ✅     | `paperless.nix`                      | Port 2892 (`paperless.home.lan`, protectedVHost), `dataDir=/mnt/pool/services/paperless`, sops admin password via upstream `LoadCredential` (superuser auto-created), OCR deu+eng, exporter on, 5 hardened units at `ioTier.background`. Pipeline proven live end-to-end (consume → OCR → archive → export). Backup: exporter manifests pool-side via dataDir                                                                                                                                       |
