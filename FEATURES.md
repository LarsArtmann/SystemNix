...
**Generated:** 2026-05-03 | **Updated:** 2026-08-14 | **Scope:** Full codebase scan
...
| flake-parts modular architecture          | ✅     | 54 modules auto-discovered (59 files total, 5 `_`-prefixed helpers skipped). Run: `nix eval .#nixosModules --apply 'x: builtins.length (builtins.attrNames x)'` |
...
| Gatus (health checks)                 | ✅     | `gatus-config.nix`                   | 93 health check endpoints (incl. nix-daemon liveness, ClickHouse ping, crash-loop/oomd/docker-restart/disk-usage/textfile-health detectors), Discord alerting, SQLite storage, port 9110, `status.home.lan` |
...
| Qmd (on-device markdown search)       | ❌ Removed | —                  | Retired — NAR hash drift across 4 deploy cycles. Replaced by smart-audio desktop daemon for audio routing. ActivityWatch remains the activity tracker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Browser History                       | ⚠️     | `browser-history.nix`                | Go CQRS/ES browser history server on port 8087 (`history.home.lan`). SQLite WAL, WebAuthn + OAuth2 (Pocket ID) dual auth. Direct TLS proxy (NOT `protectedVHost`). Agent extracts history from Firefox/Chromium profiles. OIDC oneshot via `LoadCredential`. Registration locked to single user via `MAX_USERS=1` (cqrs-htmx `MaxUsers` gate covers `/auth/register` + OAuth2 auto-provisioning + TOCTOU mutex — release/deploy chain open). **Known gaps:** 4-min startup (no `CheckpointStore`), `expires_at` session reaper schema error, OAuth2 login not manually tested |
...
| qmd                          | Node.js  | ❌ Removed | Retired — NAR hash drift across deploy cycles                                                                                                                                                 |
...
| Browser History   | StartLimit crash-loop fixed (`[Unit]` placement). Registration locked (`MAX_USERS=1`, OAuth2 + TOCTOU covered — release chain open). OTel gRPC endpoint fixed (no scheme, `d2138202`). Known gaps: 4-min startup (no `CheckpointStore`), `expires_at` session reaper schema error, OAuth2 login not manually tested, `importUsers()` path ungated | Medium   |
...
| ZFS VM configs    | VFIO PCIe passthrough PROVEN WORKING — NixOS VM (kernel 6.18.43, ZFS 2.4.3) successfully imported `datapool` mirror (2x16TB, only 21GB used — mostly disposable Docker images). SATA pool spun down (`d57c1210`). Native ZFS on host kernel 7.1 not attempted. Source: `docs/status/archived/2026-08-10_06-44_zfs-vfio-passthrough-success.md` | Low      |
...
| SigNoz dashboards | v1 dashboard API deprecated. Provisioner POST-creates with no idempotency — 251 duplicate/empty dashboards in the live DB (grows 5/deploy). 5 dashboard JSONs still in v1 flat format (v2 POSTs succeed but migrate to EMPTY dashboards). Full v2 migration research done — see TODO_LIST | Medium   |
...
| -------------------------- | ----- | ----------------------------------------------------------------------------- |
| NixOS service modules      | 54    | `ls modules/nixos/services/*.nix modules/nixos/desktop/*.nix \| grep -v '/_' \| wc -l` |
| Custom packages            | 30    | 15 mkLarsPackages + 8 pkgs/ + 7 flake-input overlays                          |
| Gatus health endpoints     | 93    | `grep -c 'name = "' modules/nixos/services/gatus-config.nix`                |
