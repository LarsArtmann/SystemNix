...
| Twenty CRM                            | ⚠️     | `twenty.nix`                         | Docker Compose (4 containers), PostgreSQL + Redis, sops secrets, daily DB backup, Caddy at crm.home.lan. Worker has `mem_limit=2g` + `NODE_OPTIONS=--max-old-space-size=1536` (prevents oomd kill loop). **Known issue:** `twenty-server` crash-loops with `FATAL: role "twenty" does not exist` (PG role mismatch). Data intact (90 tables, 17 MB).                                                                                                                                                                                          |
