...
# scripts/migrate-hot-db.sh (clickhouse precedent).
{
  flake.nixosModules.hot-db = {
    ...
    # ordering dependency.
    mountUnitName = path: lib.removePrefix "-" (builtins.replaceStrings ["/"] ["-"] path) + ".mount";
    ...
    in
      ...
      ++ lib.optionals (underHot != null) ["entry ${underHot.name} path must not be under /${hotParent}"];
  in {
