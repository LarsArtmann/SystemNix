    ...
    let
      ...
      docker = mkDockerService {
        ...
        backup = {
          ...
          execStart = "${pkgs.bash}/bin/bash -c '${pkgs.docker-compose}/bin/docker-compose -f ${composeFile} exec -T postgres pg_dump -U manifest manifest > /mnt/pool/backups/manifest/$(date +%%Y%%m%%d_%%H%%M%%S).sql && find /mnt/pool/backups/manifest -name \"*.sql\" -mtime +30 -delete'";
          schedule = "*-*-* 02:30:00";
          dir = "/mnt/pool/backups/manifest";
        };
      };
    in
    {
