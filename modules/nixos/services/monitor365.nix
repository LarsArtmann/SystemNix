...
_: {
  flake.nixosModules.monitor365 = {
    ...
    # Bootstrap env vars consumed by the server's declarative provisioning.
    bootstrapEnv = lib.optionals (cfg.server.enable && cfg.server.bootstrap.enable) (
      [
        "MONITOR365_SERVER__BOOTSTRAP__TENANT_NAME=${cfg.server.bootstrap.tenantName}"
    ...
    ssoEnv = lib.optionals (cfg.server.enable && cfg.server.sso.enable) [
      "MONITOR365_SERVER__BOOTSTRAP__SSO__ENABLED=true"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__PROVIDER=pocket-id"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__DISPLAY_NAME=${cfg.server.sso.displayName}"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__CLIENT_ID=${cfg.server.sso.clientId}"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__CLIENT_SECRET_FILE=${cfg.server.sso.clientSecretFile}"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__ISSUER=${ssoIssuer}"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__AUTHORIZE_URL=${ssoAuthorizeUrl}"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__TOKEN_URL=${ssoTokenUrl}"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__USERINFO_URL=${ssoUserinfoUrl}"
      "MONITOR365_SERVER__BOOTSTRAP__SSO__REDIRECT_URI=${cfg.server.sso.redirectUri}"
    ];
  in {
        ...
        type = lib.types.submodule {
          options = {
            ...
            bootstrap = lib.mkOption {
              type = lib.types.submodule {
                options = {
                  enable = lib.mkEnableOption "declarative first-boot provisioning (tenant + admin + API key from config)" // {default = true;};

                  tenantName = lib.mkOption {
                    type = lib.types.str;
                    default = "Monitor365";
                    description = "Initial tenant name";
                  };

                  adminEmail = lib.mkOption {
                    type = lib.types.str;
                    description = "Initial SuperAdmin email";
                  };

                  apiKeyFile = lib.mkOption {
                    type = lib.types.nullOr lib.types.str;
                    default = null;
                    description = "Path to a pre-set API key file. If null, one is generated on first boot.";
                  };
                };
              };
              default = {};
              description = "Declarative first-boot provisioning";
            };

            sso = lib.mkOption {
              type = lib.types.submodule {
                options = {
                  enable = lib.mkEnableOption "declarative SSO provisioning via OIDC";

                  issuer = lib.mkOption {
                    type = lib.types.str;
                    default = "https://auth.${domain}";
                    defaultText = lib.literalExpression ''"https://auth.''${config.networking.domain}"'';
                  ...
                  displayName = lib.mkOption {
                    type = lib.types.str;
                    default = "Pocket ID";
                    description = "Name shown on the SSO login button";
                  };
                  ...
                  clientSecretFile = lib.mkOption {
                    type = lib.types.str;
                    default = "${config.services.pocket-id.dataDir or "/var/lib/pocket-id"}/client-secrets/monitor365";
                    defaultText = lib.literalExpression ''"''${config.services.pocket-id.dataDir}/client-secrets/monitor365"'';
                    description = "Path to the OIDC client secret (generated by Pocket ID provisioning)";
                  };

                  authorizeUrl = lib.mkOption {
                    type = lib.types.str;
                    default = "";
                    description = "OIDC authorize endpoint (empty = derived from issuer as {issuer}/authorize)";
                  };

                  tokenUrl = lib.mkOption {
                    type = lib.types.str;
                    default = "";
                    description = "OIDC token endpoint (empty = derived from issuer as {issuer}/api/oidc/token)";
                  };

                  userinfoUrl = lib.mkOption {
                    type = lib.types.str;
                    default = "";
                    description = "OIDC userinfo endpoint (empty = derived from issuer as {issuer}/api/oidc/userinfo)";
                  };

                  redirectUri = lib.mkOption {
                    type = lib.types.str;
                    default = "https://monitor.${domain}/v1/auth/sso/callback";
                    defaultText = lib.literalExpression ''"https://monitor.''${config.networking.domain}/v1/auth/sso/callback"'';
                    description = "Redirect URI registered with the OIDC provider";
                  };
                };
              };
              default = {};
              description = "SSO/OIDC configuration for Monitor365 server";
            };
          };
        };
        default = {};
    ...
    config = lib.mkIf cfg.enable {
      ...
      # Allow the monitor365 user to read PocketId client secrets when SSO is enabled.
      users.users.${cfg.user}.extraGroups =
        lib.optional (cfg.server.enable && cfg.server.sso.enable) "pocket-id";
          ...
          systemd.user.services.monitor365-server = {
            ...
            Service =
              ...
              // {
                ...
                ExecStartPost = "${pkgs.curl}/bin/curl -sf --max-time 3 --retry 10 --retry-delay 2 --retry-all-errors http://127.0.0.1:${toString ports.monitor365-server}/health || true";
