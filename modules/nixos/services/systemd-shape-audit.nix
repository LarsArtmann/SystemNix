...
let
  ...
  timerOffenders =
    let
      ...
      driven = lib.filterAttrs (
        name: _t: config.systemd.services ? ${name}
      ) activeTimers;
      bad = lib.filterAttrs (
        name: _t:
        let svc = config.systemd.services.${name}; in
...
in
{
  options.services.systemd-shape-audit = {
    allowTimerRestart = lib.mkOption {
      ...
      default = [ ];
      description = ''
        Units with a matching timer whose Restart != no is DELIBBERATE:
        a bounded rapid-retry (e.g. 3x5s) that self-extinguishes via
        start-limit long before the next timer fire. Every entry MUST carry
        a justification comment where it is set.
      '';
    };
  };
