...
_: {
  flake.nixosModules.qmd-config =
    ...
    let
      ...
      inherit (sd) hardenUser mkStateDir ports serviceDefaultsUser;
          ...
          in
          {
            ...
            xdg.configFile."qmd/index.yml" =
              let
                ...
                allCollections = collectionAttrs // {
            ...
            systemd.user.services.qmd-mcp = {
              ...
              Service = lib.mkMerge [
                ...
                (hardenUser {
                  MemoryMax = "4G";
                  # qmd must read user collections (~/notes, ~/Documents, etc.)
                  # and write to ~/.cache/qmd/index.sqlite + ~/.config/qmd/.
                  ProtectHome = false;
                  # Tighten writes to ONLY the qmd data + config dirs. Outside
                  # of these, $HOME is read-only — qmd cannot scribble
                  # collections into arbitrary subdirs.
                  ReadWritePaths = [ qmdCacheDir qmdConfigDir ];
                })
                {
                  Type = "simple";
