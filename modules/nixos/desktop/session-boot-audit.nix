...
# why the boot-transaction hazard does not apply.
{
  flake.nixosModules.session-boot-audit = {
    ...
    sectionToList = section: keys:
      if section == null || !lib.isAttrs section
      then []
      else
          ...
          in [
            {
              ...
              installs =
                (
                  if u ? wantedBy
                  then valueToList u.wantedBy
                  else []
                )
                ++ sectionToList installSection installKeys;
            }
          ]
        )
        kindUnits
    ...
    parseUnitText = text: let
      ...
      fold = acc: line:
        ...
        then acc // {section = lib.substring 1 (lib.stringLength line - 2) line;}
        else let
        ...
        in
          if acc.section == "Unit" && builtins.elem key pullKeys
          then acc // {deps = acc.deps ++ tokens;}
          else if acc.section == "Install" && builtins.elem key installKeys
          then acc // {installs = acc.installs ++ tokens;}
          else acc;
    in
      builtins.foldl' fold {
        section = "";
    ...
    collectRawUnits = rawUnits:
      lib.concatLists (
        lib.mapAttrsToList (
          name: u: let
            parsed = parseUnitText (
              if u ? text && lib.isString u.text
              then u.text
              else ""
            );
            optionInstalls =
              if u ? wantedBy && u ? enable && u.enable
              then u.wantedBy
              else [];
          in [
            {
    ...
    hmUsers =
      if config ? home-manager && config.home-manager ? users
    ...
    hmKinds = lib.concatLists (
      lib.mapAttrsToList (
        _: hmUser: let
          su =
            if hmUser ? systemd && hmUser.systemd ? user
            then hmUser.systemd.user
            else null;
        in
    ...
    edgesOf = name: (pullEdges.${name} or []) ++ (installEdges.${name} or []) ++ (seedEdges.${name} or []);

    bfs = queue: seen: parents:
      if queue == []
      then parents
      else let
