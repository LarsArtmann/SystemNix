...
- **Date:** 2026-09-22 23:27 CEST · **REV 2** (2026-09-23, self-review pass)
...
| **B. License-free Node hub** ★ default | Each source repo commits its generated `catalog/` tree (git-native, reviewable — "GitHub as schema registry"); hub repo = empty EventCatalog project whose CI copies source trees into `catalog/` content dirs → `npm run build` → publish `dist/` | none | No cross-catalog validation/lockfile/provenance; ID collisions across sources must be managed by convention (domain scoping) | **DEFAULT** — 1% tier. Same one-endpoint outcome as C for a single-owner fleet |
...
| T1 | Wire EventCatalog export format into `bank-sync catalog`, commit generated `catalog/` tree | 60 | H | P0 |
| T2 | Verify the generated tree is a valid hub source (local copy-in + build smoke; also probes federate behavior later) | 45 | H | P0 |
| T3 | Scaffold hub repo `LarsArtmann/architecture-catalog` (empty catalog, pinned `@eventcatalog/core` v4 line) + source-copy layout + README | 45 | H | P0 |
| T4 | Hub CI (Forgejo runner, sources from Forgejo mirrors per F10): copy trees → `npm run build` → publish `dist/` (artifact or branch) | 90 | H | P0 |
| T5 | NixOS serving leg: DNS `catalog` + Caddy `file_server` vHost + registry entry + Gatus + tile + box-side pull-sync timer (rsync/git pull of `dist/`, atomic swap) | 90 | H | P1 |
...
| 1.5 | Commit generated `catalog/` tree + .gitignore decision | 8 | T1 |
...
| 3.1 | Create hub repo `architecture-catalog` (`create-eventcatalog --empty`) | 10 | T3 |
| 3.2 | Pin `@eventcatalog/core` v4 line in hub package.json; document why | 8 | T3 |
| 3.3 | Define sources layout (`sources/<name>/` + copy script or git subtree) | 12 | T3 |
| 3.4 | Hub README: contract, layout, upgrade path | 10 | T3 |
| 4.1 | Forgejo Actions workflow: checkout mirrors → copy trees → npm ci → build | 12 | T4 |
| 4.2 | Publish `dist/` (artifact zip or `dist` branch) | 12 | T4 |
...
| 5.2 | Hand-written Caddy `file_server` vHost (registry `vHost.layer = "none"`) | 12 | T5 |
