...
> **[docs-health 2026-09-21] RESOLVED + ARCHIVED** — partially executed with in-doc strikethroughs and superseded as a source of truth: remaining tasks were migrated into docs/todo/*.md by the 2026-09-19 TODO-system split; hot-db landed as modules/nixos/services/crush-hot-db.nix; the monitor365 step is moot (disabled).
...
~~| 2 | **Kernel 7.2.2 reboot + flm v1.0.3 retry**                                                                                      | ONE reboot closes: booted==current (rollback-generation risk), the flm XRT enumeration question, the v1.0.2 SIGABRT watch, AND provides the maintenance window for #3 and the Samsung Phase-1 initial rsync. One action, four items.          |~~ done — rebooted; flm v1.0.3 FAILED live and was reverted to v1.0.2 (2026-09-14)
...
| F36  | SIGPIPE/pipefail audit of collector pipelines (`                                                                                                                                                                                                                                          |      | true` + herestring) | 12   |
...
_Point-in-time snapshot — stale items get docs-health ANNOTATE treatment, never rewritten. New tasks surfaced during execution go to TODO_LIST.md (living source)._
