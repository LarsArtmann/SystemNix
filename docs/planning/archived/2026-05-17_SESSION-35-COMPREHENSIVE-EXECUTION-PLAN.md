...
> **[docs-health 2026-09-21] RESOLVED + ARCHIVED** — P0–P2 executed and verified (ADR-005 local-deps, update-vendor-hash.sh, workflows, archived/ consolidation, attic instead of Cachix); P3 items never done and now moot.
...
~~| 4.3  | Set up Cachix binary cache for private LarsArtmann repos. Create cachix cache, configure `nix.settings.substituters` in flake.nix, push initial builds.                                                                                                                                                                                                  | 10m    | Faster builds           | SystemNix |~~ done — landed as the self-hosted attic cache, not Cachix
