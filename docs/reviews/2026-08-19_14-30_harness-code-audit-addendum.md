...
spurious — those are _not_ being assembled. They are spec/design
...
`[server]`, `[project]`; no `[mcp.X]`). What ADR-0010 _actually_
...
_config-level_ validation against the user-facing list happens in
...
truly unbuilt layers — but the _adapter's_ projection half is the
...
_only_ on the raw TOML parser struct
...
the _default_ (true for profiles/autostart, false otherwise), not
...
| Distiller-as-harness pattern              | No code (V06) — the _as-designed_ separation is correct even if unimplemented                                                         | **Pattern stands; implementation pending.**                                        |
...
The doc-driven review got the _directions_ right: scheduler is
thin, daemon owns everything, persistence splits across ring/log/state,
config is hand-editable. The code-driven review gets the _magnitudes_
