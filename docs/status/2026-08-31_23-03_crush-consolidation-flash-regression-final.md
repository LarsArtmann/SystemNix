...
1. **Deploy 3 (llamacpp + flash) is BLOCKED** — pre-deploy §10 phantom-metric gate fails on `pool_usb_recovery_device_errors` / `pool_usb_recovery_members_present`: the parallel session's new Gatus checks (their commits `2f84bd47`/`545a102b`) declare metrics that only their own, same-config collector will emit — a §10 chicken-and-egg with no bypass flag in deploy.sh (line 5 gate, line 366 abort). Their transient flake-check failure (VM test `/dev/vdb` assertion) was fixed by them within minutes; the §10 pair was still red at 21:45 and not re-verified since (their docs sweep is in flight). The unblock belongs to their session: allowlist the two metric names as pending-new, or teach §10 "skip metrics whose emitter exists in the NEW toplevel but not the live system".
2. **The flash/llamacpp crushrc is proven but undelivered** — scratch-tested + eval-clean in the working tree, NOT yet committed (daemon sweep pending amid their docs reorg) and NOT deployed. Live crush currently has NO flash and NO llamacpp (crush.json already deleted): zero functional loss today (:8899 has no listener; flash requests were falling back anyway), but every crush session started until the deploy lands lacks both.
