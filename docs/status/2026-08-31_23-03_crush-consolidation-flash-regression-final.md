...
## CLOSE-OUT ROUND (2026-09-02 00:1x) — execution of the pending work

1. **Per-provider key probes: 4/4 keys authenticate.** gemini (`gemini-3-flash-preview`) → KEY-OK; kimi-coding (`kimi-for-coding`) → KEY-OK; minimax (`MiniMax-M2.7`) → authenticates, then `rate_limit_error` 2056 "Token Plan usage limit reached" — a BILLING state, not a key failure; zai/flash already proven by the user's own session. minimax needs a plan top-up/upgrade to be usable.
2. **§10 gate:** the parallel session's `f00a33ec` shipped the `KNOWN_NEW_METRICS` allowlist (dated, explicit "one-deploy loan, not a museum" retirement culture, same-day sweeps already practiced). The general emitter-grep rule was EVALUATED AND DEFERRED: it would downgrade absent-metric failures to warns whenever a name appears in any unit script (weaker gate) and rewrites a freshly-hardened shared script. Revisit only if the allowlist shows maintenance pain.
3. **`scripts/crush-rc-test.sh`** — new harness: isolated `XDG_CONFIG_HOME` rc load test + `--probe`/`PROBE_MODEL` one-completion key check + `EXTRA=` candidate-line testing. Both modes verified live; probe re-proved flash (`served RC-OK`).
4. **`docs/services/crush.md`** — runbook: provider table, key onboarding (public-key sops staging trick), rotation, 4-step verification suite, gotchas (no `reasoning_levels` flag, discover_models default, session-DB snapshots, placeholder-inert pattern).
5. **fish_history leak purged** — 3 entries carried LIVE keys (minimax ×2 via `set`/`export`, zai-prefixed inside a `sops --set` for hermes' glm_api_key). Deleted via `printf 'all\n' | fish -c "history delete --contains …"`; final masked audit = 0. Doctrine added to AGENTS.md Critical Rules: never inline secret values in command lines — interactive shells included.
6. **`~/tmp/go-lint` canary explained** — 380M regrowth was a transient dead-mount fallback (fish guard redirected the env; wrapper respected it), not a routing bug: buildcache was I/O-healthy at check time and `/mnt/buildcache/golangci-lint` (+`-analysis`) grew to 1.2G as the primary. Fallback contents cleared; regrowth-while-buildcache-healthy = the investigation trigger.
7. **AGENTS.md** — two new Critical Rules (land-then-remove + entity inventory; no secret values in command lines, interactive included) + crush section notes (session-restart generalization, minimax quota, runbook pointer).
8. **Cleanups:** stale mimo doc trashed; recent_models has no mimo entries; no live tooling references the deleted crush.json (only historical session-DB content); `~/.config/go/env.local` has no references (the git.nix hit was the `.env.local` dotenv gitignore — false positive).
9. **Verification:** formatter CI 0-changed (one transient failure was the parallel session's mid-edit state), `nix flake check --no-build` all checks passed, harness script bash -n clean.
10. **Still open (user decisions):** key rotation (minimax quota makes it timely), `model large` pin, upstream charm catalog PR for flash, macOS parity, :8899 service-ization, `.crush/crush.db` 2.55G investigation.
