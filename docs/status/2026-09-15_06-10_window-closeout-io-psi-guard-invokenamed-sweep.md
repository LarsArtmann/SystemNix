...
| --- | --- | --- | --- |
| 000001a09ceb… | Reviewer finding (high): the gitleaks commit-message rewrite (`8e8be78c`) never landed on branch history; the fabricated-gitleaks commit `120ada36` was still reachable; the status report claimed otherwise | `695ffda8` | Re-performed as a `git filter-branch --msg-filter` message-only rewrite; amended commit `0ae59e3b` IS an ancestor of HEAD; report carries an inline CORRECTION block plus corrected §-claims and verification transcript |
