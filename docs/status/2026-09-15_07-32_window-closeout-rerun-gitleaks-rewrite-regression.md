...
4. *In passing (adjacent, not this window's five):* sibling batch task `000001a09d06cf0bc…` (enqueued 01:07:44) dead-lettered after 3 failed verify runs on go-taskqueue's own suite (`internal/session` package build failure) — flagged for the go-taskqueue owner.
