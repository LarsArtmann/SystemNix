...
2. Update PapDashboard `AGENTS.md` with the notify rendering contract.
...
3. Add optional test-insight trigger (CLI flag or env) for deploy-time Discord verification.
4. Consider adding a `post-deploy-check.sh` assertion: papdashboard webhook config present when enabled.
