...
1. Commit + push `image-updates.yml`, `check-image-updates.sh`, images bump — activate the daily enforcement
2. Twenty v2.31.1: read release notes for breaking migrations; decide single-jump vs stepwise (2.7→~2.12→~2.18→2.31); verify a fresh pg_dump first; then bump
...
7. `nixpkgs-compat`-style: first manual run of the workflow via `workflow_dispatch` after push to prove the issue-creation path works end-to-end
...
14. Watch Gatus/Discord for alert noise from tonight's double-restart (should be none)
...
18. CHANGELOG: decide whether it's maintained or dead; if dead, delete or archive to stop pretending
...
22. DiscordSync API still in startup backfill (SKIP in smoke) — expected, but confirm it converges
