...
1. `nix flake lock --update-input browser-history` in SystemNix to pick up auth pages
2. `nh os switch .` to deploy browser-history with auth pages
3. `sudo systemctl restart caddy` to load the `history.home.lan` vHost
4. Open `https://history.home.lan/register` in browser — verify registration form renders
...
9. Add `sudo systemctl restart caddy` to `scripts/deploy.sh` after `nh os switch`
10. Fix `pre-deploy-check.sh` phantom metric check (`fail()` → `warn()`)
...
23. Read `nix/agent-module.nix` in browser-history repo
24. Wire agent token via sops secret
25. Enable agent in `configuration.nix`
26. Deploy agent to evo-x2
27. Verify agent sends data to `/ingest` endpoint
28. Consider macOS agent deployment
29. Test agent with Chrome/Brave/Firefox history DBs
...
32. Verify Gatus health check passes for `history.home.lan`
33. Add Gatus alert for auth endpoint failure rate
34. Add Homepage tile verification (already added — verify it links correctly)
...
42. Add OIDC/Pocket ID as alternative auth method
