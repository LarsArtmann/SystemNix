...
**Mission:** User challenged the zero-media verdict: _"Your job is not done until you find me 1 item from immich or paperless — on the HDD or SSD, in some ZFS datastore or Kubernetes volume!"_ Then rejected the evo-x2 live immich (17 GB, all 2026-dated) as "NOT the right immich server."
...
4. **PG probe role guess**: connected with `-U postgres` though the compose file I had _just read_ specified `immich`/`paperless` users. One wasted cycle ignoring my own evidence.
