...
**Resolution 2026-09-06 (docs-health pass):** root cause fixed in code (upstream untagged at the time, deployed via the later flake bumps); the LIVE outage (re-auth) is the TODO P1 `InboxClean main OAuth re-consent` row; the monitoring-lies are documented in AGENTS + CHANGELOG. Archived as the incident record.
