...
- **ExecStartPre/TimeoutStartSec eval audit**: Can you introspect `config.systemd.services` for the *combined* effect of `mkMerge` blocks at eval time? `ExecStartPre` might be set in one merge block and `TimeoutStartSec` in another — the eval-time attrset would show both, but the logic needs to handle `mkIf`-conditional services (disabled services have empty `serviceConfig`).
