...
~~1. **Push browser-history commits** — `cd /home/lars/projects/browser-history && git push origin master`~~ done
~~2. **Fix nixpkgs tarball regression** — `bash scripts/fix-nixpkgs-lock.sh` in SystemNix~~ done
~~3. **Update SystemNix flake.lock** for browser-history to latest HEAD (`3f8830c`) after push~~ done
~~4. **Run `nix flake check --no-build`** to confirm regression is resolved~~ done
~~5. **Deploy** — `nix run .#deploy` or `nh os switch .`~~ done
~~6. **Verify `browser-history-oidc-setup.service`** completed — `systemctl status browser-history-oidc-setup.service`~~ done
~~7. **Verify browser-history sees Pocket ID** — Check env vars: `systemctl show browser-history --property=Environment`~~ done
~~8. **Verify `/register` page shows "Login with Pocket ID" button** — `curl -sk https://history.home.lan/register | grep -i pocket`~~ done
...
~~10. **Add Gatus health check** for browser-history OAuth2 endpoint in `gatus-config.nix`~~ done
~~11. **Verify `history.home.lan` is in `dnsLocal.localSubdomains`** — DNS resolution check~~ done
~~12. **Add browser-history to Homepage dashboard** if not already there~~ done
13. **Add browser-history SQLite DB to backup-coordination** in `configuration.nix`
~~14. **Update SystemNix AGENTS.md SSO table** — browser-history is now Layer 1 native OAuth2~~ done — AGENTS.md:206
~~15. **Update SystemNix AGENTS.md browser-history section** — Document Pocket ID integration, secret bridging, SSL_CERT_FILE~~ done — AGENTS.md:148-160
...
~~18. **Deploy browser-history agent** — Server has no data source yet~~ done — 2,927 events synced
...
> **PARTIALLY RESOLVED — OAuth2 integration complete. Server running with pocket-id provider.** Deployment items (1–8), monitoring (10–12), AGENTS.md (14–15), agent (18) are done. Still open: item 9 (OAuth2 browser flow test), item 13 (backup coordination — in TODO_LIST), item 17 (registration lock), items 19–50 (hardening, code quality, architecture, documentation, testing, security).
