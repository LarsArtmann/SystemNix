...
6. ~~**Consider the second-order effects of cgroup changes.** Lowering MemoryHigh prevents one crash mode but may introduce another (EOF errors). The fix only works as a _system_ — code fix + cgroup limits + monitoring — not as individual layers.~~ done — analyzed; upstream code fix mitigates EOF death-loop
