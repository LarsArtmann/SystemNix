...
**Activate & verify**
~~1. Reboot → verify zram device ≈ 46.9 GiB (`memoryPercent = 50` base = post-carveout MemTotal).~~ done — reboot 2026-09-05 13:16 (1 GiB BIOS-floor carveout): zram auto-scaled to **62.2 GiB** at 50% of MemTotal 124.3 GiB, verified live (AGENTS GPU/ZRAM sections)
...
**Docs**
~~30. CHANGELOG entry for round 2 (churn, cap, per-key cooldown, duration metrics).~~ done — CHANGELOG Unreleased `SEV1 alerting overhaul + zram 30%→50%` (2026-09-06 docs-health pass)
~~31. Annotate both 2026-09-02 status reports after reboot + first-churn-cycle outcomes (docs-health ANNOTATE).~~ done — this annotation pass, 2026-09-06
