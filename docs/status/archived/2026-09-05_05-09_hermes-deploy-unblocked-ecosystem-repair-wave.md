...
**Resolution 2026-09-06 (docs-health pass):** deployed + verified (91 PASS / 3 baseline FAILs); CHANGELOG `Hermes v0.21.0 + ecosystem mass-bump repair wave` + `sops-key-audit` + `browser-history provisioning`. The two forgotten verifications + the NEW hermes cron errors are harvested into TODO_LIST (2026-09-06 pass; cron row added). Execution-complete.
