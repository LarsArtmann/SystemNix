...
| 5 | `000001a0981cd72c…` | Review fix: wrong red-era nixpkgs pin citation (`e5bdc4a`)           | `120ada36`                                            | Done — pin corrected to `34ab99075`; ~~gitleaks hex trap defused~~ RETRACTED 2026-09-14: the "gitleaks flags 40-char hex" rationale was FABRICATED (`gitleaks detect --no-git` on the bare SHA → no leaks found; the rule keys on `sq0atp-` prefixes), and the fabricated sentence was removed from reachable history via message-only rewrite (`0ae59e3b`) — see `docs/status/2026-09-13_04-22_task-000001a09873d0ca8d87606b52037ab2158d.md` |
...
5. ~~**Hex-literal hygiene**: never paste full 40-char SHAs into tracked docs (gitleaks scans the whole exported index — one violation blocks ALL commits repo-wide). Abbreviate in tree; full form recoverable via the cited command.~~ RETRACTED 2026-09-14: the premise is false (bare git SHAs do not trip gitleaks — see the 04-22 report). Abbreviating SHAs remains good style, not a gate requirement.
...
3. ~~**Gitleaks vs hex shapes:** abbreviation-only for commit SHAs in tracked docs (current, safe, slightly lossy) or a bounded `.gitleaks.toml` allowlist for SHA-shaped backtick strings (convenient, marginally weakens the secret scan)? Recommendation: abbreviation-only.~~ MOOT 2026-09-14: there is no gitleaks-hex rule to policy around (the claim was fabricated and retracted); keep citing SHAs abbreviated as style.
...
_Point-in-time snapshot — 2026-09-13 ~04:00 CEST. Report per repo task-queue convention; nothing pushed._
