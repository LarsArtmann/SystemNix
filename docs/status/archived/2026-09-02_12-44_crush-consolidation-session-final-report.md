...
~~1. minimax: top up Token Plan or retire the provider (also resolves its row in the runbook table).~~ done — RETIRED (disabled) by user decision 2026-08-31: `providers.minimax.disabled = true` in the crush-config repo; quota exhausted = billing state, not a bad key (AGENTS Crush section)
...
~~10. TODO_LIST backfill of c1-c10 (coordinate with the Pareto session that owns the file).~~ done — TODO_LIST P3 `crush-config phase 2` row carries the surviving follow-ups
...
~~16. Commit the staged dotfiles-repo untracking (`crushrc`) — user or daemon.~~ done — the 2026-09-05 extraction made the crushrc symlink UNTRACKED + gitignored in the local dotfiles repo (the file's own comment finally satisfied)
...
## ADDENDUM (2026-09-02 14:10) — minimax disabled + two deploy-blocking recoveries

1. **minimax DISABLED (user decision "disable for now")** — crushrc gained `provider add minimax --disable true` after the key injection (merge keeps the rendered sops key for re-enabling). Verified: session selection rejects with "model not found"; NOTE `crush models` still lists the catalog's minimax entries — the CLI ignores disabled providers (documented in the runbook). Runbook + AGENTS.md updated. Re-enable = delete one line + restart sessions.
2. **Deploy blocker #1: bank-sync vendorHash stale** — the lock had moved to upstream `c6342780` (ref=master moved after the parallel session's 901978e measurement), FOD failed "hash mismatch" in installPhase. Re-measured per doctrine (`vendorHash = ""` → build → `got:`): `sha256-xkA6DNlqMPYDbGFGAxTb3AEVLOMl/ieI9SwgEPv9DOI=`; comment updated. LESSON: a ref=master pin makes every measured vendorHash a one-rev loan.
3. **Deploy blocker #2: hermes crash-loop (ModuleNotFoundError: hermes_state_registry)** — batch lock-only update `a7cf841d` (Sep 2 00:11, 12 inputs) moved hermes-agent `59c5bd09`→`ab9866bc`; the new upstream dropped the py-module from the sealed venv (the exact recurrence AGENTS.md predicted for the registration_lifecycle class). Rolled the flake.lock node back to `59c5bd09` (lastModified 1788195655) — first deploy after the rollback: **85 PASS / 0 FAIL, hermes running**. Upstream follow-up needed: check why `hermes_state_registry` vanished (py-modules drop or rename) before the next hermes bump.
4. **Observation (another session's domain): `cache.home.lan` resolution is flaky** — narinfo lookups failed with "Could not resolve hostname" repeatedly during builds (nix disables the cache for 60s and falls back — non-fatal but slows builds). The local-DNS blind-spot workstream (`9e266e6c`) should cover verifying attic/dnsblockd health.
5. **Deploy count this session: 2 blocked-then-recovered, 1 green (85 PASS / 0 FAIL).** Working tree carries: home.nix (minimax disable), bank-sync.nix (measured hash), flake.lock (hermes rollback), runbook + AGENTS.md + report edits.

---

**Resolution 2026-09-06 (docs-health pass):** the consolidation's endgame was SUPERSEDED and completed by the 2026-09-05 crush-config extraction (`docs/status/archived/2026-09-05_21-14_crush-config-extraction-status-self-review.md` — the repo now owns the crushrc via `homeManagerModules.crush`; byte-identical 1609-model parity proven). Key rotation + minimax retirement are tracked in TODO_LIST P0 (persistent nag) / AGENTS Crush section (minimax DISABLED 2026-08-31). Remaining unstruck items are consider-level; archived as superseded-and-complete.
