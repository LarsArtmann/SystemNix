...
| # | Item | What's missing |
|---|------|----------------|
| 1 | **Interactive verification of the binds** | Validated syntactically + config loaded clean, but I cannot press keys from an SSH session. User must confirm `Mod+Ctrl+L` jumps to the TV and `Mod+Ctrl+Shift+L` carries a column over. | ~~superseded in part — the 2026-08-16 niri multiscreen session re-planned the bind matrix (TODO_LIST Priority 5 "Now" batch awaits a 2nd identical LG)~~ |
| 2 | **Workspace→output mapping is a UX guess** | I inferred "work on DP-1, chat+media on DP-2" from refresh rates (60Hz desk vs 30Hz TV). Not confirmed with the user. Trivially reversible (one attrset). | ~~confirmed since — AGENTS.md documents DP-1 = main/browser/dev, DP-2 = chat/media as the standing layout~~ |
| 3 | **Monitor-move coverage** | I bound `move-column/window-to-monitor-*` but NOT `move-workspace-to-monitor-*`. Defensible (workspace moves are rarer) but it's a gap in the matrix. | ← open — TODO_LIST Priority 5 (optional `move-workspace-to-monitor` in the "Now" batch) |
| 4 | **DP-2 fallback behavior** | Wiki/source-verified that missing `open-on-output` targets fall back to primary — but not empirically tested by unplugging the TV. | ← open — untracked |
...
3. Unplug/replug DP-2 — verify chat/media migrate to DP-1 and return (5 min) ← open — untracked
4. Trigger a test notification — confirm popup on focused screen only (1 min) ← open — user-side, untracked
...
6. Add `move-workspace-to-monitor-*` binds if workspace-carrying turns out to be wanted (5 min) ← open — TODO_LIST Priority 5 ("Now" batch)
7. If TV popups (OSD/toast) annoy: set DMS `screenPreferences` for those components (5 min) ← LEAVE ALONE (contingent on user complaint)
8. Distinct DMS bar config for DP-2 (TV-friendly: larger, fewer widgets) (30 min) ← open — untracked
9. Add eval-time assertion: `workspaces.*.open-on-output` ∈ `outputs` keys (15 min) ← open — untracked
10. Flake check running `niri validate` against the built evo-x2 config (20 min) ← open — untracked
11. `niri-msg` wrapper that auto-discovers `NIRI_SOCKET` from `/run/user/$UID/` (10 min) ← open — untracked
12. Verify rofi `monitor=-1` under the Sway backup session (5 min, low priority) ← open — untracked (low)
