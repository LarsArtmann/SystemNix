...
**Resolution 2026-09-06 (docs-health pass):** niri fix committed `e2ee6182` + deployed (Existed(4) churn gone); flm corpse documented reboot-only (AGENTS); CHANGELOG `Deploy Exited(4) root-caused pair`. The collector .tmp audit is a TODO P1 row. Execution-complete.
