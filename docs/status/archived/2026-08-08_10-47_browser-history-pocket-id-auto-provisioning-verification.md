...
1. **Push 8 browser-history commits**: `cd /home/lars/projects/browser-history && git push origin master`
2. **Fix nixpkgs tarball regression**: `bash scripts/fix-nixpkgs-lock.sh`
3. **Update SystemNix flake input to browser-history HEAD**: `GIT_CONFIG_GLOBAL=/dev/null nix flake lock --update-input browser-history`
4. **Verify flake.lock nixpkgs type is github**: `python3 -c "import json; print(json.load(open('flake.lock'))['nodes']['nixpkgs']['original']['type'])"`
5. **Run flake check**: `nix flake check --no-build`
6. **Deploy**: `nix run .#deploy`

### Post-Deploy Verification

7. **Check oneshot service**: `sudo systemctl status browser-history-oidc-setup.service`
8. **Verify env vars loaded**: `sudo systemctl show browser-history --property=Environment`
9. **Verify OIDC env file exists**: `sudo cat /var/lib/browser-history/oauth2-secrets.env` (should contain `OAUTH2_POCKET_ID_CLIENT_ID` and `OAUTH2_POCKET_ID_CLIENT_SECRET`)
10. **Check browser-history logs for OIDC discovery**: `sudo journalctl -u browser-history -n 50 --no-pager`
11. **Verify "Login with Pocket ID" button**: `curl -sk https://history.home.lan/register | grep -i pocket`
...
19. **Update SystemNix AGENTS.md** browser-history section with the OAuth2 integration details
