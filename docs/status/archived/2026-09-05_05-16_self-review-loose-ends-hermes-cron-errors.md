...
~~1. **hermes cron `systemd-run --user --scope` errors** — pick fix path (module: hermes-user lingering/user-manager or a scope alternative; or upstream issue) and land it.~~ routed — NEW TODO_LIST P1 row `Hermes v0.21.0 cron scheduler ERRORS` (2026-09-06 docs-health harvest)
...
**Resolution 2026-09-06 (docs-health pass):** the honest re-audit's findings are all routed: hermes cron errors → NEW TODO P1 row (2026-09-06 harvest), EMAIL_HOST → TODO P1, BuildFlow flip-flopper + go-output v0.37.1 → TODO P6 rows, reboot → P0. Archived as the record that caught them.
