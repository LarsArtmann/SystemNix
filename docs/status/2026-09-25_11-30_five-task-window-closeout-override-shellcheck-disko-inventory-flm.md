...
**Harvest ledger (post-authoring):** 8 of the 10 (f) items were appended to TODO_LIST.md (the PSI-preamble script and `preferStaticEmulators` items were held back to respect the 10-item cap and are recoverable from this report); all 3 (g) questions appended as BLOCKED rows (offsite-borg `mkDefault` → storage, shellcheck source + STATE-doc placement → pipeline). Docs-health pass: two historical reports annotated inline with the disko-gate closure (06-03 §d1, 07-31 §d1); three CHANGELOG Unreleased entries added (damage-set inventory + cross-links, disko-gate environmental root cause + fix chain, shellcheck proof batch); no reports archived (the intermediate disko-task reports remain cited from TODO_LIST row 43's evidence chain); README/FEATURES/ROADMAP/AGENTS verified current for this docs-only window (AGENTS.md was updated by the window's own tasks). Noted in passing: `docs/status/README.md` (the referenced archive-convention file) does not exist.
