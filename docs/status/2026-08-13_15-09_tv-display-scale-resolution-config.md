...
3. HaGeZi blocklist 404 — `HaGeZi-dga7-raw` fetch returns 404 from all mirrors, aborting the build
4. Failed systemd services (browser-history-agent, nix-build-cleanup)
...
`platforms/nixos/system/boot.nix` has uncommitted changes from another session — ZRAM config changed from 17% to 30% memoryPercent and swappiness documentation updated. This is NOT my change; I did not touch it.
...
The `HaGeZi-dga7-raw` derivation fails with 404 from all mirrors. This blocks ALL deploys. Not investigated — likely an upstream URL change or temporary outage.
...
Not verified whether the declarative `outputs` config in `niri-wrapped.nix` will correctly override niri's auto-detection on next niri restart. The config evaluates correctly but has never been deployed + restarted to confirm.
...
**This is the most critical issue.** `nix run .#deploy` cannot complete due to a HaGeZi blocklist 404 error. This means NO NixOS configuration changes can be deployed to evo-x2. Every declarative change is stuck in limbo until this is fixed. The runtime `niri msg output` workaround is a band-aid, not a solution.

### 2. Disk space is chronically critical
Root filesystem at 90-94% on a 723GB drive. The deploy hard-fails at >=95%. Two GC runs freed 34 GiB but the system is already back at 90%. This is a systemic problem — the nix store is growing faster than GC can keep up, and there's no automated GC schedule configured.
...
### Config
4. **Niri `outputs` should have been in the config from day one** — Relying on auto-detection means display config is non-reproducible. Adding it now is correct.
...
### Infrastructure
8. **HaGeZi blocklist URLs need monitoring** — A 404 on a blocklist fetch should not block ALL deploys. Consider a fallback or cached version.
9. **Automated nix GC schedule** — The system has no `nix.gc` auto-optimize-store schedule. Disk fills to 95%+ between manual GC runs.
...
1. Fix HaGeZi blocklist 404 — investigate the URL, update or add fallback
2. Get a successful `nix run .#deploy` to activate the niri outputs config permanently
3. Verify niri outputs config survives niri restart (after deploy succeeds)
4. Set up automated nix GC schedule (`nix.gc.automatic = true`) to prevent disk filling
5. Reset failed services: browser-history-agent, nix-build-cleanup
...
17. Verify all LarsArtmann flake inputs have `follows` on go-nix-helpers
18. Run `nix flake check --no-build` after flake.lock changes
19. Audit all flake inputs for duplicate/locked entries
20. Consider `nix flake update --recreate-lock-file` to start fresh (nuclear option)
...
29. Consider `nix.settings.auto-optimise-store = true` for hardlink dedup

### Pre-existing issues noticed but not investigated
30. Monitor365 endpoint is down (port 9191 not responding — metrics skipped)
31. browser-history-agent.service is in failed state
32. nix-build-cleanup.service is in failed state
33. boot.nix has uncommitted ZRAM changes (17% → 30%, swappiness 10 → 150) from another session
34. The HaGeZi 404 affects multiple list variants: dga7, doh, gambling, dyndns, native-lgwebos
...
43. The `pre-deploy-check.sh` should warn at 90% (currently only at 95% hard-fail)
44. Add a health check for "time since last successful deploy" (alert if >7 days)

### Follow-up from this session
45. Confirm the niri outputs config is in the next successful deploy diff
46. Remove the temporary runtime `niri msg output` overrides after deploy succeeds
47. Update the status report with "deploy verified" once it succeeds
