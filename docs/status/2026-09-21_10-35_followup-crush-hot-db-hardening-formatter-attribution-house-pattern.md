...
**Poller outcome (16:02):** GAVE UP after 2h — no calm window. 24 polls (14:06–16:01), io PSI some avg60 ranged 23–84% (best single dip 23% at 14:16, immediately rebounded), guard events never below 3 per 10 min. The storm has run ~42h continuous and is NOT abating. The `crush-hot-db` VM test therefore remains unrun — batch it into the user's deploy calm-window (`heavy-job nix build --no-link .#checks.x86_64-linux.crush-hot-db`; first execution of the rewritten test, a failure there = previous session's bug). Prognosis for tonight's 23:00 btrbk window: at current storm levels the send likely gets guard-stopped again and the Sep 22 00:28 verify FAILs (by design, Discord-pages) — the accepted self-heal stance anticipated exactly this.
