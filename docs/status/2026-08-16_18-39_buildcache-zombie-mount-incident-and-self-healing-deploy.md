...
| Time | Event |
|------|-------|
| 15:57–16:33 | JMicron JMS567 enclosure (`usb 8-1`, `152d:0567`) **disconnects 9 times in 36 min**; each drop produces *write* I/O errors on ext4 |
| 16:35 | Different device appears on `usb 6-1` (RTL9210B — the *other* SSD enclosure) |
...
2. **Kernel:** `mount(2)` resolves `/dev/disk/by-id/...` to a device **number**; the VFS stores only `major:minor`. On reconnect the drive gets a new number; the by-id symlink correctly repoints but **cannot re-point the established mount**. Deterministic paths solve *recovery*, not *prevention*.
...
### b) Partially Done
- **fsck not run:** the fs took *write* errors during flaps and is now mounted RW again with `data=writeback`. Cache-only data, consumers verify hashes — acceptable, but `e2fsck -f` on the partition during the next unplug is proper hygiene.
