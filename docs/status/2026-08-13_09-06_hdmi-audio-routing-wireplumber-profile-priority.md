...
2. **Added `wireplumber.extraConfig` to `audio.nix`** with:
   ...
   - `device.restore-profile = false` to prevent WirePlumber from overriding the declarative priority with whatever was last selected at runtime via `wpctl set-profile`
3. **Applied runtime fix** via `wpctl set-profile 43 <index>` so audio works immediately without requiring a deploy
4. **Flipped priority twice** -- initially set HDMI 2 (monitor) as priority, then user said "for now I need it on my TV", so switched to HDMI 3 (TV) as priority
...
1. **Runtime audio verified but not auditorily confirmed** -- I confirmed the sink switched via `wpctl status` (HDMI 3 is the active sink, volume 1.00), but never played a test sound. `speaker-test` and `amixer` were not on PATH. Could have used `pw-cat` or a Helmholtz test tone to confirm audio actually reaches the TV speakers
2. **Config file written but NOT deployed** -- the Nix change is in the working tree but `nix run .#deploy` was NOT run. The runtime fix (`wpctl set-profile`) is temporary; it will survive until WirePlumber restarts or the system reboots, at which point the declarative config takes over (once deployed)
3. **Volume set to 100%** -- `wpctl set-volume 51 1.00` was applied, but the WirePlumber config has `device.routes.default-sink-volume = 0.064` (6.4%) as the default. The `device.restore-routes` setting (still `true`) means WirePlumber will restore the last known volume, so 100% should persist after deploy. But if routes are not restored, audio will be very quiet on next boot

---

## C) NOT STARTED

1. **AGENTS.md update** -- The audio routing gotcha (WirePlumber profile priority for multi-HDMI setups) should be documented in the "Desktop (DMS / Quickshell / Helium)" or a new "Audio" section of `AGENTS.md`. Currently there is no mention of WirePlumber or audio routing anywhere in the project docs
...
4. **Deploy** -- No `nix run .#deploy` was executed
...
1. **Scope `device.restore-profile = false` to just the Radeon device** -- Investigate whether WirePlumber 0.5 supports per-device settings overrides, or write a small Lua script that only ignores stored profiles for `alsa_card.pci-0000_c5_00.1`. The global disable is a sledgehammer
2. **Remove em dashes from comments** -- Replace with commas/parentheses per project conventions
3. **Make audio output selectable** -- Instead of hardcoding which HDMI port has priority, consider a NixOS option like `services.audio-config.hdmiOutput` that lets the user pick "monitor" or "tv" without editing the priority list. Or even better, a runtime switcher script
4. **Document the WirePlumber profile priority mechanism in AGENTS.md** -- The fact that `find-preferred-profile` runs AFTER `find-stored-profile` (so stored profiles override priority rules unless `device.restore-profile = false`) is a critical non-obvious gotcha
...
6. **Test audio output after changes** -- Find a way to play a test tone. `pw-cat` might work, or install `alsa-utils` in the system config for `speaker-test`
...
| 2 | Run `nix fmt` to format `audio.nix` | HIGH | 1 min |
| 3 | Deploy the audio config change: `nix run .#deploy` | HIGH | 10 min |
| 4 | Verify audio works on TV after deploy (play a YouTube video or similar) | HIGH | 2 min |
| 5 | Update AGENTS.md with WirePlumber audio routing gotcha (profile priority vs stored profile ordering) | MEDIUM | 10 min |
| 6 | Investigate per-device `device.restore-profile` scoping instead of global disable | MEDIUM | 30 min |
| 7 | Add `services.audio-config` NixOS option for HDMI output selection (monitor vs TV) | LOW | 30 min |
...
| 13 | Verify the WirePlumber config survives a reboot (after deploy) | HIGH | 5 min |
...
| 16 | Consider whether `device.restore-routes = true` (still enabled) causes volume issues | MEDIUM | 10 min |
| 17 | Add a Gatus check or metric for "default audio sink is the expected device" | LOW | 30 min |
| 18 | Pre-commit hook: run it to verify no other issues (deadnix, statix, gitleaks) | MEDIUM | 2 min |
| 19 | Verify the config file naming (`51-` prefix) follows any SystemNix conventions | LOW | 5 min |
| 20 | Consider making the HDMI port mapping (extra1=HDMI2, extra2=HDMI3) a comment in the config | LOW | 2 min |
