...
**Honest bottom line:** the original ask (latest go-taskqueue, deployed) is _built and committed everywhere upstream + SystemNix tree_, but NOT deployed — the Go 1.27.1 ecosystem wave turned a one-input bump into a five-repo fix sweep, and the freshly-discovered `overview` upstream eval breakage is the last gate. Signoz sonic patches are in place and compile-proven but unconfirmed end-to-end and uncommitted.
