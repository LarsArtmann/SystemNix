...
*(40 items; the remaining 10 slots intentionally unfilled — padding to 50 would create fake work.)*

## g) QUESTIONS I CANNOT FIGURE OUT MYSELF

1. **Deploy now or wait?** The IO storm is driven by *your* workloads (two 92%-CPU llama-servers + a parallel agent session's builds). I cannot know whether that parallel work is nearly done (wait) or shoppable. If you want the switch regardless: `DEPLOY_FORCE_PRESSURE=1 nix run .#deploy` is the sanctioned override; the gate has refused it 3× with real-storm verdicts.
...
*Report ends. Waiting for instructions. Section (f) is the docs-health HARVEST input.*
