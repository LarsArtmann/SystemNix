...
- **`StartLimitBurst`/`StartLimitIntervalSec` in `serviceConfig` = SILENTLY IGNORED (CRITICAL)** — In systemd 261+, these directives are ONLY valid in the `[Unit]` section. Placing them in `serviceConfig` (which maps to `[Service]`) causes systemd to silently ignore them with a warning: `Unknown key 'StartLimitIntervalSec' in section [Service], ignoring`. This means services with `Restart=on-failure` will restart INFINITELY with no limit — the exact bug that caused the 2026-08-11 WDT crash chain (browser-history-agent reached 592 restarts). **Correct placement:** Use NixOS top-level options `systemd.services.<name>.startLimitBurst` and `systemd.services.<name>.startLimitIntervalSec` (camelCase, outside `serviceConfig`), or use `unitConfig.StartLimitBurst`. The `service-defaults.nix` helper explicitly documents this (lines 21-27) but browser-history.nix violated it. The bug is now fixed in browser-history.nix — verify other modules don't have the same pattern. All other services (forgejo, pocket-id, caddy, dns-blocker, oauth2-proxy, niri) correctly use `unitConfig`.

- **systemd-oomd kills nix-daemon during builds (2026-08-12 outage)** — During large nix builds, nix-daemon is the top memory consumer under `/system.slice` (4-8G peak, 20+ child processes). oomd's pressure threshold (`boot.nix`) targets it, SIGKILLs all build processes. Because nix-daemon is **socket-activated**, every pending nix command immediately re-triggers the socket → starts nix-daemon → oomd kills it again → rapid restarts → **start-limit-hit** → daemon permanently dead → `Connection refused` on ALL nix operations. Fix: `ManagedOOMPreference = "omit"` on nix-daemon's serviceConfig (`networking.nix`), same pattern as PMA. oomd will target other services first; nix-daemon is critical infrastructure that must not be killed mid-build. Recovery: `sudo systemctl reset-failed nix-daemon.service && sudo systemctl start nix-daemon.service`. **Threshold raised from 50%/20s to 60%/30s (2026-08-14):** The previous 50%/20s killed nix-daemon mid-build (65% pressure during legitimate 4-8 GB build spike) and Twenty worker in steady-state. 60%/30s tolerates transient spikes from nix builds, AI model loads, and Docker restarts while still catching genuine exhaustion. Per-slice `ManagedOOMMemoryPressureLimit` also raised from 50% to 60% for all slices
...
- **ClickHouse `background_pool_size` sanity check trap (2026-08-13 crash)** — Reducing `background_pool_size` below 16 triggers a cascade of ClickHouse startup sanity checks (`Code: 36 BAD_ARGUMENTS`). ClickHouse validates that multiple merge_tree settings (`number_of_free_entries_in_pool_to_execute_mutation`, `number_of_free_entries_in_pool_to_lower_max_size_of_merge`, `number_of_free_entries_in_pool_to_execute_optimize_entire_partition`, etc.) are each less than `background_pool_size * background_merges_mutations_concurrency_ratio` (default ratio=2). With pool=2, capacity=4, but the defaults for these settings are 20/8/25 — ALL fail. Each fails one-at-a-time (whack-a-mole through deploys). The fix: keep `background_pool_size` at default (16). The thread savings from reducing `background_schedule_pool_size` (128→8), `background_fetches_pool_size` (8→1), etc. are far larger (~145 threads) than the ~14 saved by reducing the merge pool. **Do NOT reduce `background_pool_size` without overriding ALL dependent merge_tree settings.**
- **ClickHouse `background_pool_size` sanity check trap** — Upstream sets `Type=notify` but binary never calls `sd_notify`. SystemNix overrides `Type=exec`. Remove when upstream adds sd_notify.
- **PMA page-cache death-loop (2026-08-09 crash)** — PMA reads 260+ git repos during discovery, charging ~16 GB of page cache to its cgroup. Without `MemoryHigh`, the kernel reclaimed pages but PMA immediately re-read them → 91% CPU thrash loop with `memory.events max = 27,312` but `oom_kill = 0` (page cache is reclaimable, never OOM-killed). System-wide PSI hit 95% → kernel freeze → sp5100-tco WDT reset. **Fix:** `MemoryHigh=6G` (throttle via direct reclaim), `MemoryMax=8G` (OOM-kill+restart if exceeded), `CPUQuota=200%` (cap at 2 cores), `MemorySwapMax=0` (only 370 MB anon), `PMA_COMMITTER_WORKERS=2` (halve concurrent pressure). Upstream fix: `isNothingToCommit()` in committer treats TOCTOU "clean working tree" races as `StatusSkipped` not `StatusFailed`, avoiding unnecessary cooldown + retry cycles. Gatus alerts on `system_service_cpu_over_threshold` and `system_service_memory_over_threshold` for PMA at 2-min intervals. **Memory.events monitoring:** `system-health` collector scrapes `/sys/fs/cgroup/system.slice/<svc>/memory.events` for all monitored services, emits `system_service_memory_events_max` (raw counter), `system_service_memory_events_high` (per-service threshold flag at 100), and `system_memory_events_any_high` (summary flag — 1 if ANY service exceeds threshold). Gatus "Memory Events Thrash" check alerts on the summary metric. The collector runs as root with `ProtectSystem=full` (which only makes `/usr`/`/boot`/`/efi`/`/etc` read-only — `/sys/fs/cgroup` is NOT affected). See `docs/crash-analysis-2026-08-09.md`.
...
- **ClickHouse `background_pool_size` sanity check trap** — Reducing `background_pool_size` below the default (16) triggers cascading sanity check failures in `MergeTreeSettingsImpl::sanityCheck()`: `number_of_free_entries_in_pool_to_execute_mutation` (default 20), `number_of_free_entries_in_pool_to_lower_max_size_of_merge` (default 8), `number_of_free_entries_in_pool_to_execute_optimize_entire_partition` (default 25) — all must be less than `background_pool_size * background_merges_mutations_concurrency_ratio` (default 2). An eval-time assertion in `signoz.nix` catches `background_pool_size=2`. The other 4 pool reductions (`background_schedule_pool_size=8`, `background_buffer_flush_schedule_pool_size=4`, `background_move_pool_size=2`, `background_fetches_pool_size=1`) are safe — they don't have sanity-checked dependents.
