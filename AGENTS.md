...
- **Deploy-time E2E smoke** (`scripts/post-deploy-check.sh`): `curl --max-time 240 http://127.0.0.1:52625/v1/models` through the public socket, enable-gated. It is the ONLY functional gate for this endpoint (Gatus cannot probe :52625) — it would have caught the dead-endpoint class within one deploy. Note it cold-pins the 13.6 GB model on every deploy (connection restarts the idle clock). Pre-deploy check #12 (ExecStart existence) catches the phantom-binary class before switch.
