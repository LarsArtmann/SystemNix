...
- **nixpkgs tarball lock regression** — `flake.nix` has eval-time `nixpkgsTarballGuard`. If `nix flake update` (all inputs) rewrites nixpkgs to tarball type, both `nix flake check` and `nix eval` fail.
