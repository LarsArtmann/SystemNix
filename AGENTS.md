...
- **PMA page-cache death-loop containment** — PMA reads 260+ git repos during discovery, charging ~16 GB of page cache to its cgroup. Without `MemoryHigh`, reclaim loops at 91% CPU (`memory.events max` climbing, `oom_kill = 0` — page cache is reclaimable) until system-wide PSI freezes the kernel. Contained by: `MemoryHigh=6G`, `MemoryMax=8G`, `CPUQuota=200%`, `MemorySwapMax=0`, `PMA_COMMITTER_WORKERS=2`, `ManagedOOMPreference=omit`. The `system-health` collector emits `system_service_memory_events_{max,high}` metrics with a Gatus "Memory Events Thrash" alert (collector runs as root with `ProtectSystem=full` — `/sys/fs/cgroup` unaffected). Full narrative: `docs/crash-analysis-2026-08-09.md`
