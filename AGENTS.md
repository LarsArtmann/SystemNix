...
- **Browser History: registration locked to single user (2026-08-14)** — `POST /auth/register` was open to anyone on LAN. Now gated by cqrs-htmx `ServiceConfig.MaxUsers` (0=unlimited, N=N users max). The gate checks `readModel.Count() >= maxUsers` in `Service.Register()` and returns `ErrRegistrationClosed` (HTTP 403). browser-history exposes `MAX_USERS` env (default 1). SystemNix sets `MAX_USERS=1` explicitly. Changing this requires deploying new cqrs-htmx + browser-history tags and bumping the flake input.
