...
| Forgejo sops key | The sops secret key is `forgejo_token` (not `gitea_token`). Renamed via migration script during Gitea→Forgejo migration. | Resolved |
...
| Forgejo runner token | Token generated inline in runner's `ExecStartPre` via `+` prefix (root) + `runuser -u forgejo`. No separate token service. Must use `utils.escapeSystemdPath hostName` for service name override — nixpkgs module escapes hyphens in hostnames (`evo-x2` → `evo\x2dx2`). Without escaping, the override creates a separate service that doesn't merge with nixpkgs module's service. `EnvironmentFile` uses `-` prefix to tolerate missing file on first start. Migration marker (`.forgejo-migrated`) forces re-registration once after Gitea→Forgejo migration. | Resolved — inline token generation |
