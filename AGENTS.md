...
- **Mirror dirs are created by the mount-gated `google-sync-dirs` oneshot** (atticd-storage-dir pattern), NOT by an ExecStartPre mkdir inside the sync unit: systemd sets up mount namespacing BEFORE ExecStartPre, and a `ReadWritePaths` entry pointing at a missing path aborts the unit with `status=226/NAMESPACE` (live incident 2026-08-18 00:33 — a force-enable verification build was accidentally deployed enabled and crash-looped 4× on this until the next deploy). deploy.sh restarts the oneshot after switches
