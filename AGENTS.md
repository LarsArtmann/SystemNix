...
**Module:** `modules/nixos/services/google-sync.nix` (`services.google-sync`) — rclone one-way sync of Google Drive to `/mnt/pool/backups/google-drive` every 5 min (`interval` option). **Ships DISABLED** until the OAuth token is filled in (placeholder in sops `google_sync_rclone_config`, `platforms/nixos/secrets/google-sync.yaml`); a `google-sync-config-check` ExecStartPre fails fast with the go-live checklist pointer when the token is still a placeholder, and `rclone listremotes` parse-checks the INI (both live-tested against synthetic configs).
