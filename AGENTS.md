...
| `/tmp` tmpfs size capped at 8 GiB | `boot.tmp.useTmpfs` defaults tmpfs size to 50% of RAM (~47 GiB). go-build caches and dev tool temp files accumulated 16+ GiB in a single 21-hour session. Now capped at 8 GiB via explicit `fileSystems."/tmp"` with `options = ["size=8G"]`. `useTmpfs` must be `false` when defining the mount manually (setting both causes an eval conflict) |
