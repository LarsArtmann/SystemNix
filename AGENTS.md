...
| dnsblockd embedded resolver ≠ unbound replacement (yet) | dnsblockd has a full recursive resolver (`semihalev/sdns`) with DNSSEC, DoT, DoH, and caching (F60-F70, feature-flagged `dns_enabled`). SystemNix still runs the split architecture: unbound on :53 (with local zones, upstream DoT to Mullvad/Quad9, LAN ACLs) + dnsblockd as block-page HTTP server only. Three gaps block migration: (1) no local zone support (`home.lan.` static records), (2) no upstream DoT forwarding (sdns does direct root recursion — ISP sees all queries in cleartext), (3) no LAN access-control lists. See ROADMAP.md for the migration path |
...
| Unbound RSS 8x cache size | Unbound configured with only 192 MiB of caches (64 MiB msg + 128 MiB rrset) but shows **1.5 GiB RSS**. DNSSEC key cache and NXDOMAIN cache are **unbounded by default** — `key-cache-size` and `neg-cache-size` now explicitly set to 16 MiB each. With `prefetch=true` + `prefetch-key=true` + `qname-minimisation=true`, unbound generates many background queries that populate these caches |
...
| monitor365 SQLite sequences upstream bug | monitor365-server's Rust schema migration uses `CREATE SEQUENCE` (PostgreSQL syntax) which fails on SQLite with `Binder Error: SQLite databases do not support creating sequences`. This is an **upstream bug** in the monitor365 repo, not fixable in SystemNix. The flake input was reverted from revCount 2214 to 2194 (commit `0f0a05e`) to restore the working SQLite schema. When re-updating monitor365, verify the server starts without this error before deploying |
