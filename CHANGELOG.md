...
- **Browser-history registration lock (single-user)** — `POST /auth/register` was open to anyone on LAN. Now gated by `MaxUsers` config in cqrs-htmx `ServiceConfig` (0=unlimited, N=N users max; generalizes the requested `registration_open` boolean — auto-closes after the Nth user, no flag-flip deploy needed). Gate checks `readModel.Count() >= maxUsers` in `Service.Register()` and returns `ErrRegistrationClosed` (HTTP 403). browser-history exposes `MAX_USERS` env (default 1); SystemNix `browser-history.nix` sets `MAX_USERS=1` explicitly. 3 tests added upstream (max reached, unlimited, multi-user); full `usermgmt` suite passes. **Deploy blocker:** requires new cqrs-htmx + browser-history tags, then flake input bump
...
- **Registration lock — coverage caveat** — the `MaxUsers` gate closes `POST /auth/register` only. OAuth2 (Pocket ID) first-login auto-provisioning in cqrs-htmx `OAuth2Service` dispatches `RegisterUserCmd` directly and is NOT gated (a second user can still be created via "Login with Pocket ID"); the count check is also advisory (TOCTOU between count and dispatch). Closing the gap is tracked in TODO_LIST (LarsArtmann Apps)
