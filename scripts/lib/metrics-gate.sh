...
metrics_gate_classify_absence() {
  ...
  elif echo "$BANKSYNC_METRICS" | grep -qw "$metric" && [ "$BANKSYNC_UP" = false ]; then
    warn "Metric '$metric' absent (bank-sync :8097 down — infrastructure signal, not a phantom metric; the gatus Bank-Sync checks own the outage alerting)"
