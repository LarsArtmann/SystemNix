...
# The crushrc is executed at EVERY crush start; a bad statement aborts the
# whole config load. This smoke proves the deployed surface is the HM one
# (store symlinks, generated header) and that it still LOADS (isolated
# crush models) with model IDENTITY asserted (the 2026-08-31 lesson: never
# trust a reply/model list without knowing which entity served it).
echo ""
echo "=== Crush Config ==="
CRUSH_RC="${XDG_CONFIG_HOME:-$HOME/.config}/crush/crushrc"
if [ -e "$CRUSH_RC" ]; then
