...
[ -b "$DEVICE" ] || {
  echo "FAIL: $DEVICE not a block device"
  exit 1
}
case "$DEVICE" in /dev/zram* | /dev/loop*)
  echo "REFUSING zram/loop"
  exit 1
  ;;
esac
...
[ -n "$SAMPLE_KB" ] && [ "$SAMPLE_KB" -gt 0 ] || {
  echo "FAIL: store sample empty — delete $STORE_SAMPLE and retry"
  exit 1
}
...
setup_fs() {
  ...
  case "$fstype" in
  ext4)
    mkfs.ext4 -F -q -L nixfs "$DEVICE" >/dev/null
    mount -o noatime "$DEVICE" "$MNT"
    ;;
  xfs)
    mkfs.xfs -f -m reflink=1 -L nixfs "$DEVICE" >/dev/null
    mount -o noatime "$DEVICE" "$MNT"
    ;;
  btrfs)
    mkfs.btrfs -f -L nixfs "$DEVICE" >/dev/null
    mount -o noatime,compress=zstd,space_cache=v2 "$DEVICE" "$MNT"
    ;;
  esac
}
...
metadata_test() {
  ...
  [ ! -e "$MNT/m" ] || {
    echo "FAIL: metadata cleanup incomplete"
    exit 1
  }
}
...
store_copy_test() {
  ...
  mkdir -p "$MNT/store"
  t=$({ time (rsync -a --recursive --files-from="$STORE_SAMPLE" /nix/store/ "$MNT/store/" && sync); } 2>&1 | awk '/real/ {print $2}')
